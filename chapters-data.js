...
if project is None:
  ...
  Answer --> Eval[Grounding / retrieval eval]`,
    ...
    return sorted(scores, key=scores.get, reverse=True)` }),
      ...
      s('03.6 高阶 RAG：何时增加结构，何时保持简单', ['Contextual Retrieval 在索引阶段为每个 chunk 补充其父文档/章节语境，缓解孤立片段歧义；Late Interaction 保留 token 级表示，提升精排细粒度但增加索引与查询成本。Multimodal RAG 需要把图、表、布局和文字的证据关系一并保存，不能只对 OCR 文本做向量化。', 'GraphRAG 适合问题必须跨实体/关系多跳时：先构建实体关系图或社区摘要，再与普通检索并行召回。不要把它当通用精度按钮；图抽取错误、更新复杂和路径膨胀会使简单知识库反而变差。实时数据用 CDC 触发 parse/chunk/embed/index generation，并通过 alias 原子切换；删除和 ACL 变更必须优先传播到所有索引与缓存。'], { callout: '升级准则：只有离线失败样本能证明“文本块不足以提供所需关系、模态或时效性”时，才引入 GraphRAG、多向量或复杂索引。' })
    ],
